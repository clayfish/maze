...
public class Maze30 {
    ...
    public static class MazePanel extends JPanel {
        ...
        /**
         * Βοηθητική κλάση που καθορίζει ότι τα κελιά θα ταξινομούνται
         * με βάση τη μεταβλητή τους dist
         */
        ...
        private class RepaintAction implements ActionListener {
            @Override
            public void actionPerformed(ActionEvent evt) {
                // Εδώ αποφασίζουμε αν μπορούμε να συνεχίσουμε
                // την αναζήτηση με 'Κίνηση' ή όχι.
                // Για την περίπτωση των αλγόριθμων της κ. Κεραυνού
                // εδώ έχουμε το 2ο βήμα τους:
                // 2. Εάν ΑΝΟΙΚΤΕΣ = [], τότε τερμάτισε. Δεν υπάρχει λύση.
                if ((dijkstra.isSelected() && graph.isEmpty()) ||
        ...
        public MazePanel(int width, int height) {
            ...
            programer = new JLabel("Προγραμματιστής: Νίκος Κανάργιας  ver. 3.0", JLabel.CENTER);
        ...
        private void expandNode(){
            ...
            if (dijkstra.isSelected()){
                ...
                for (Cell v: neighbors){
                    // 20: alt := dist[u] + dist_between(u, v) ;
                    int alt = u.dist + dist_between(u,v);
                    // 21: if alt < dist[v]:
                    if (alt < v.dist){
                        // 22: dist[v] := alt ;
                        v.dist = alt;
            ...
            // Ο χειρισμός των υπόλοιπων αλγόριθμων
                ...
                ArrayList<Cell> succesors = createSuccesors(current, false);
        ...
        private ArrayList<Cell> createSuccesors(Cell current, boolean makeConnected){
            ...
            if (diagonal.isSelected()){
                ...
                if (r > 0 && c > 0 && room[r-1][c-1] != OBST &&
                    ...
                    // αναδρομική μέθοδο dfs() η οποία δημιουργεί κατά την
         ...
         */
        private int dist_between(Cell u, Cell v){
            int dist;
            int dx = u.col-v.col;
            int dy = u.row-v.row;
            if (diagonal.isSelected()){
        ...
        /**
         * Αναδρομική μέθοδος που προσθέτει στην λίστα που περιέχει
         * τους κόμβους του γραφήματος μόνο εκείνα τα κελιά
         * τα οποία ανήκουν στην ίδια συνεκτική συνιστώσα με τον κόμβο v.
         * 
         * Πρόκειται για υπολοίηση μιας αναζήτησης σε βάθος.
         * όπως περιγράφεται στο σχετικό άρθρο της Wikipedia και πάλι.
         ...
         * με αυτήν της αρχικής θέσης του ρομπότ.
                ...
                for (int c = 0; c < columns; c++) {
                    ...
                    if ((room[r][c] == TARGET && found)  || room[r][c] == ROUTE  || 
                        ...
                        // Αλλιώς ...
                            ...
                            if (room[r][c] != FRONTIER || !dijkstra.isSelected()) {
                                g.setColor(Color.WHITE);
